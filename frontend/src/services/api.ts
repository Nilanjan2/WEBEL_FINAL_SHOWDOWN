...
export async function fetchEmailsByCategory(category: string): Promise<BackendEmail[]> {
  const response = await fetch(`${API_BASE_URL}/emails?category=${encodeURIComponent(category)}`);
  if (!response.ok) throw new Error('Failed to fetch emails');
  return response.json();
}

// Fetch all emails (by fetching each category)
...
export async function generateReply(emailContent: string, emailSubject: string = '', sender: string = ''): Promise<string> {
